...
***NOTE: IF YOU DO NOT CORRECTLY EXPORT THE `EvilDragon` CLASS, THE 
MOCHA TESTS WILL THROW A 'TypeError` AND NOT RUN***

4. Additionally, add two(2) instance methods to the `EvilDragon` class. 
    a. A method named `dontInviteThemOverForDinner` that will console.log out each of 
    the Evil Dragon's evil doings (stored in an array) as a separate console.log
    for each evil thing in the format `${name} will ${evilDoing}`
    b. A second method named "burnsNemesis" that will return a string using `name` and 
...
const enderDragon = new EvilDragon("Ender Dragon", "black", ["spit a fireball at Steve",
    "rule over all the Endermen", "destroy all blocks in its way"], "Steve");
...
const drogon = new EvilDragon("Drogon", "black-red",["eat all your livestock",
    "burn down your castle in Westeros","burn your father and brother"],"Night King");
console.log(drogon);
