...
1. Require with CommonJS the Dragon class from the `./problem-01-make-dragons.js` module.
...
*** NOTE: IF YOU DO NOT CORRECTLY EXPORT THE `FriendlyDragon` CLASS, THE 
MOCHA TESTS WILL THROW A 'TypeError` AND NOT RUN ***

4. Additionally, add two(2) instance methods to the `FriendlyDragon` class. 
    a. A method named `hasLifeGoals` that will console.log each of the FriendlyDragon's
    life goals (stored in an array) as a separate console.log for each life goal in the format
    `${name} likes to ${lifeGoal}`
    b. A second method named `helpsPeople` that will return a string using the `name` and `friend` 
    property values formatted as `${name} helps his friend ${friend}`.
...
const puff = new FriendlyDragon("Puff", "green", ["live by the sea", 
    "frolick in the autumn mist", "help small schildren"], "Jackie Paper");
console.log(puff);
...
const toothless = new FriendlyDragon("Toothless", "black", ["save the town of Burke", 
    "fly with a kid on his back", "hang out with vikings"], "Hiccup");
console.log(toothless);
toothless.hasLifeGoals();
console.log(toothless.helpsPeople());
console.log(toothless.breathesFire());   //Friendly dragons can still breathe fire!
...
//  Toothless helps his friend Hiccup
